...
-- Date: 19/03/13
-- Time: 10:28 PM
-- Debugger taken from: http://files.luaforge.net/releases/clidebugger/clidebugger/Updated/debugger.lua
--
...
--local dbg_win 			= require("scripts/platform/windows")
...
local function debugger_loop(ev, vars, file, line, idx_watch)
    ...
    while true do
        ...
        -- If enter pressed then work out what was entered
        local line = "" --io.read("*line")
        while(line == "") do
            -- Render the Output of the loop
            line = gDebugPanel:Update()
            gDebugPanel:Render()
        end

        if line == "debug_quit" then line = "run"; gDebugPanel:Finish() end
        if line == nil then line = 'exit' end
